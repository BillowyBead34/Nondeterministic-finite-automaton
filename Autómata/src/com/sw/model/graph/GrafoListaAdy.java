package com.sw.model.graph;

import com.sw.model.graph.exceptions.ArcoNoExistenteException;
import com.sw.model.graph.exceptions.GrafoLlenoException;
import com.sw.model.graph.exceptions.NodoNoExistenteException;
import com.sw.model.graph.exceptions.NodoYaExisteException;
...
public class GrafoListaAdy<E> extends Graph<E> {
    ...
    @Override public void addNode(E name) throws GrafoLlenoException, NodoYaExisteException {
    ...
    @Override public void removeNode(E name) throws NodoNoExistenteException {
        ...
        for (int i = 0; i < getNumeroVertices(); i++)
            for (Iterator<Arch<?>> iterator = tablaAdy.get(i).getArchs().iterator(); iterator.hasNext();) {
                Arch<?> arch = iterator.next();
                if (arch.getDestino() == node)
                    iterator.remove();
            }
    }

    @Override public void addArch(E origen, E destino, Object dato) throws NodoNoExistenteException {
    ...
    @Override public void removeArch(E origen, E destino) throws ArcoNoExistenteException, NodoNoExistenteException {
        if (!existeConexion(origen, destino))
        ...
        for (Iterator<Arch<?>> iterator = nodoOrigen.getArchs().iterator(); iterator.hasNext();) {
            Arch<?> arch = iterator.next();
    ...
    @Override public boolean existeConexion(E origen, E destino) throws NodoNoExistenteException {
        Node<E> nodoOrigen = getNode(origen);
        Node<E> nodoDestino = getNode(destino);
    ...
    @Override public int numNode(E name) {
        for (int i = 0; i < getNumeroVertices(); i++)
    ...
    @Override public Node<E> getNode(E name) {
    ...
    @Override public List<Node<E>> getVertices() {
        return tablaAdy;
    }

    @Override public List<Arch> getArcos() {
